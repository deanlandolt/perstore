...
var maxLimit = settings.maxLimit;
var defaultLimit = settings.defaultLimit;
...
exports.Model = function(store, schema) {
	...
	if (!schema.maxLimit) {
		schema.maxLimit = settings.maxLimit;
	}
	/*if (!schema.defaultLimit) {
		if (defaultLimit) schema.defaultLimit = settings.defaultLimit;
	}*/
...
exports.getAllProperties = function(model) {
	// TODO resolve if $ref?
	if (typeof model === "string") model = getModelByPath(model);
	var properties = model.properties;
	if (model["extends"]) {
		var inherited = exports.getAllProperties(model["extends"]);
		Object.keys(inherited).forEach(function(key) {
			if (!properties[key]) properties[key] = inherited[key];
		});
	}
	return properties;
}
